...
<script type="text/javascript">
   ...
   $(function(){
	...
	function doCmd(){
		...
		var data=PublicAjax('/grid_zz/index.php/Home/Index/doRegister',$('#form1').serialize());
		//alert(data);
		if(data==-5){
			alert('请确认没有修改手机号码，且浏览器没有禁用cookie!');
		}else if(data==-4){
			alert('验证码已过期，请重新获取！');
		}else if(data==-3){
			alert('该手机号已经注册！');
...
<body>
 	...
 	<div class="register">
    	<div class="r-title r-top"><h1>快速注册 永久免费</h1></div>
    	<div class="r-main">
    		<form id="form1">  			
        	<ul>
                <li><span><label for="account">注册账号<font color="red">*</font></label></span><input name="account" id="account" type="text" class="inputText" want="yes" title="注册账号" /></li>
                <li><span><label for="password">密 　码<font color="red">*</font></label></span><input name="password" id="password" type="password" class="inputText" want="yes" title="密码" /></li>
                ...
                <li style="padding:0;"><span></span><input name="button" type="button" value="注册" class="button" onclick="doCmd()" /></li>
            </ul>
    		</form>
        </div>
