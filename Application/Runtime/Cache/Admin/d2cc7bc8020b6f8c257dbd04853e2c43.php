...
function doDelete(){
	...
	//alert(ids);
	location.href='/grid_zz/index.php/Admin/Keyword/delete?ids='+ids;
}
...
<div id="div_right">
	<div class="right_top">
				...
				<div class="title">
					<a href="/grid_zz/index.php/Admin/Keyword/keywordUI?type=<?php echo ($type); ?>"  class="btn btn-green">新增</a> 				
		...
		<div class="develop"  >
					...
					<tbody id="goaler">
						<?php if(empty($keywordList)): ?><tr>
							<td colspan="5" align="center"><font color="red">暂无记录</font></td>
						</tr><?php endif; ?>
						
						<?php if(is_array($keywordList)): foreach($keywordList as $key=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>">
						         <td class="thcheckbox"><input type="checkbox" name="chk" id="chk" onclick="selTr(this);" ></td>						      
						         <td><?php echo ($vo["name"]); ?></td>						   					 					         			 					         
						         <td>
						         	<a href="/grid_zz/index.php/Admin/Keyword/keywordUI?id=<?php echo ($vo["id"]); ?>&type=<?php echo ($type); ?>">编辑</a>&nbsp;					         	
						         </td>						 					         
