...
function doDelete(){
	...
	//alert(ids);
	if(confirm("确定删除吗？")){
	   location.href='/grid_zz/index.php/Admin/News/delete?ids='+ids;
	 }
}
...
<div id="div_right">
	<div class="right_top">
				...
				<div class="title">
					<a href="/grid_zz/index.php/Admin/News/newsUI?type=<?php echo ($type); ?>"  class="btn btn-green">新增</a> 				
		...
		<div class="develop"  >
					...
					<tbody id="goaler">
						<?php if(empty($newsList)): ?><tr>
							<td colspan="5" align="center"><font color="red">暂无记录</font></td>
						</tr><?php endif; ?>
						
						<?php if(is_array($newsList)): foreach($newsList as $key=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>">
						         <td class="thcheckbox"><input type="checkbox" name="chk" id="chk" onclick="selTr(this);" ></td>
						         <td><?php echo ((isset($vo["title"]) && ($vo["title"] !== ""))?($vo["title"]):$vo['subject']); ?></td>
						         <td><?php echo ($vo["intime"]); ?></td>
						         <td><?php echo ($vo["source"]); ?></td>						 					         
						         <td>
						         	<a href="/grid_zz/index.php/Admin/News/newsUI?id=<?php echo ($vo["id"]); ?>&type=<?php echo ($type); ?>">编辑</a>&nbsp;
